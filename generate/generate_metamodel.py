...
def generate_metamodel(spec: openapi_parser.specification.Specification) -> list[classes.Entity]:
    ...
    generator = templates.TemplatedGenerator("model.py.jinja2")
    code_dir = pathlib.Path(__file__).parent / "../jubladb_api/metamodel"
    with open(code_dir / "model.py", "w") as f:
        f.write(generator.render({"entities": entities}))
    return entities
