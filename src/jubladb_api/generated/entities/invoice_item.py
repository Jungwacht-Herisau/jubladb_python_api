# DO NOT EDIT THIS FILE, it is auto-generated!
import jubladb_api.core.metamodel_classes
import jubladb_api.core.base_entity
import keys


# @formatter:off


class InvoiceItem(jubladb_api.core.base_entity.BaseEntity):
    def __init__(
        self,
        id_: int,
        invoice_id: int,
        name: str,
        description: str,
        unit_cost: float,
        vat_rate: float,
        cost: int,
        count: int,
        cost_center: str,
        account: str,
        invoice: keys.InvoiceKey,
    ):
        super().__init__(id_)

        self._invoice_id = invoice_id
        self._name = name
        self._description = description
        self._unit_cost = unit_cost
        self._vat_rate = vat_rate
        self._cost = cost
        self._count = count
        self._cost_center = cost_center
        self._account = account

        self._invoice = invoice

    @property
    def invoice_id(self) -> int:
        return self._invoice_id

    @property
    def name(self) -> str:
        return self._name

    @property
    def description(self) -> str:
        return self._description

    @property
    def unit_cost(self) -> float:
        return self._unit_cost

    @property
    def vat_rate(self) -> float:
        return self._vat_rate

    @property
    def cost(self) -> int:
        return self._cost

    @property
    def count(self) -> int:
        return self._count

    @property
    def cost_center(self) -> str:
        return self._cost_center

    @property
    def account(self) -> str:
        return self._account

    @property
    def invoice(self) -> keys.InvoiceKey:
        return self._invoice

    @property
    def key(self) -> keys.InvoiceItemKey:
        return keys.InvoiceItemKey(self._id)

    @property
    def meta(self) -> jubladb_api.core.metamodel_classes.Entity:
        return jubladb_api.generated.metamodel.ENTITIES["invoice_item"]

    @classmethod
    def from_json(cls, json_data: dict):
        if json_data.get("type", None) != "invoice_items":
            raise ValueError("Invalid data type")
        return cls(
            id_=int(json_data["id"]),
            invoice_id=json_data["attributes"]["invoice_id"],
            name=json_data["attributes"]["name"],
            description=json_data["attributes"]["description"],
            unit_cost=json_data["attributes"]["unit_cost"],
            vat_rate=json_data["attributes"]["vat_rate"],
            cost=json_data["attributes"]["cost"],
            count=json_data["attributes"]["count"],
            cost_center=json_data["attributes"]["cost_center"],
            account=json_data["attributes"]["account"],
            invoice=keys.InvoiceKey(
                cls._access_json_single_relation_id(json_data, "invoice", "invoices")
            ),
        )


# @formatter:on
