# DO NOT EDIT THIS FILE, it is auto-generated!
from jubladb_api.core.metamodel_classes import *
import typing

ENTITIES = {
    "additional_address": Entity(
        url="",
        name_singular="additional_address",
        name_plural="additional_addresses",
        string_representation_attributes=[
            "contactable_id",
            "street",
            "housenumber",
            "zip_code",
            "town",
        ],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="public",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="contactable_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="contactable_type",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="address_care_of",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="street",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="housenumber",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="postbox",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="zip_code",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="town",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="country",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[],
        includeable=[],
    ),
    "additional_email": Entity(
        url="",
        name_singular="additional_email",
        name_plural="additional_emails",
        string_representation_attributes=["contactable_id", "email"],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="public",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="contactable_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="contactable_type",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="email",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="mailings",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="invoices",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[],
        includeable=[],
    ),
    "course": Entity(
        url="",
        name_singular="course",
        name_plural="courses",
        string_representation_attributes=["name"],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="group_ids",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="type",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="kind_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="description",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="application_conditions",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="motto",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="cost",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="location",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="application_opening_at",
                type_=AttributeType.DATE,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="application_closing_at",
                type_=AttributeType.DATE,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="application_contact_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="external_application_link",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="maximum_participants",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="created_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="updated_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="state",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="training_days",
                type_=AttributeType.FLOAT,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="applicant_count",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="participant_count",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="minimum_participants",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="number",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="teamer_count",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="contact",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="kind",
                related_type_singular="event_kind",
                related_type_plural="event_kinds",
                to_many=False,
            ),
            RelationType(
                relation_name="dates",
                related_type_singular="date",
                related_type_plural="dates",
                to_many=True,
            ),
            RelationType(
                relation_name="participations",
                related_type_singular="event_participation",
                related_type_plural="event_participations",
                to_many=True,
            ),
            RelationType(
                relation_name="leaders",
                related_type_singular="person",
                related_type_plural="people",
                to_many=True,
            ),
        ],
        includeable=[],
    ),
    "date": Entity(
        url="",
        name_singular="date",
        name_plural="dates",
        string_representation_attributes=["event_id", "start_at", "end_at"],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="event_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="location",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="start_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="finish_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="event",
                related_type_singular="event",
                related_type_plural="events",
                to_many=False,
            )
        ],
        includeable=[],
    ),
    "event_guest": Entity(
        url="",
        name_singular="event_guest",
        name_plural="event_guests",
        string_representation_attributes=[],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="first_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="last_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="nickname",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="company_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="company",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="email",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="address_care_of",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="street",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="housenumber",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="postbox",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="zip_code",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="town",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="country",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="language",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="phone_number",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[],
        includeable=[],
    ),
    "event_kind_category": Entity(
        url="/api/event_kind_categories",
        name_singular="event_kind_category",
        name_plural="event_kind_categories",
        string_representation_attributes=["label"],
        allowed_operations=[Operation.GetList, Operation.GetSingle],
        attributes=[
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="order",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[],
        includeable=[],
    ),
    "event_kind": Entity(
        url="/api/event_kinds",
        name_singular="event_kind",
        name_plural="event_kinds",
        string_representation_attributes=["label"],
        allowed_operations=[Operation.GetList, Operation.GetSingle],
        attributes=[
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="short_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="general_information",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="application_conditions",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="minimum_age",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="created_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="updated_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="kind_category",
                related_type_singular="event_kind_category",
                related_type_plural="event_kind_categories",
                to_many=False,
            )
        ],
        includeable=["kind_category"],
    ),
    "event_participation": Entity(
        url="/api/event_participations",
        name_singular="event_participation",
        name_plural="event_participations",
        string_representation_attributes=[],
        allowed_operations=[Operation.GetList, Operation.GetSingle],
        attributes=[
            Attribute(
                name="event_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name="event_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="participant_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name="participant_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="participant_type",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name="participant_type",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="application_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="active",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="qualified",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="additional_information",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="created_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="updated_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="event",
                related_type_singular="event",
                related_type_plural="events",
                to_many=False,
            ),
            RelationType(
                relation_name="participant",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="roles",
                related_type_singular="role",
                related_type_plural="roles",
                to_many=True,
            ),
        ],
        includeable=["event", "participant", "roles"],
    ),
    "event_role": Entity(
        url="",
        name_singular="event_role",
        name_plural="event_roles",
        string_representation_attributes=[],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="participation_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="type",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="participation",
                related_type_singular="event_participation",
                related_type_plural="event_participations",
                to_many=False,
            )
        ],
        includeable=[],
    ),
    "event": Entity(
        url="/api/events",
        name_singular="event",
        name_plural="events",
        string_representation_attributes=["name"],
        allowed_operations=[Operation.GetList, Operation.GetSingle],
        attributes=[
            Attribute(
                name="group_ids",
                type_=AttributeType.INTEGER,
                array=True,
                optional=False,
                sortable=False,
                filter_name="group_id",
                filter_types=["eq", "not_eq"],
            ),
            Attribute(
                name="type",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=False,
                filter_name="type",
                filter_types=["eq"],
            ),
            Attribute(
                name="kind_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=True,
                sortable=False,
                filter_name="kind_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="description",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="application_conditions",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="motto",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="cost",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="location",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="application_opening_at",
                type_=AttributeType.DATE,
                array=False,
                optional=True,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="application_closing_at",
                type_=AttributeType.DATE,
                array=False,
                optional=True,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="application_contact_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=True,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="external_application_link",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="maximum_participants",
                type_=AttributeType.INTEGER,
                array=False,
                optional=True,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="created_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="updated_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=False,
                filter_name="updated_at",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
        ],
        relations=[
            RelationType(
                relation_name="contact",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="kind",
                related_type_singular="event_kind",
                related_type_plural="event_kinds",
                to_many=False,
            ),
            RelationType(
                relation_name="dates",
                related_type_singular="date",
                related_type_plural="dates",
                to_many=True,
            ),
            RelationType(
                relation_name="participations",
                related_type_singular="event_participation",
                related_type_plural="event_participations",
                to_many=True,
            ),
        ],
        includeable=["contact", "kind", "dates", "participations"],
    ),
    "group": Entity(
        url="/api/groups",
        name_singular="group",
        name_plural="groups",
        string_representation_attributes=["name"],
        allowed_operations=[Operation.GetList, Operation.GetSingle],
        attributes=[
            Attribute(
                name="name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="name",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="short_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="short_name",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="display_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="display_name",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="description",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="description",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="layer",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=True,
                filter_name="layer",
                filter_types=["eq"],
            ),
            Attribute(
                name="parent_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=True,
                filter_name="parent_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="layer_group_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=True,
                filter_name="layer_group_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="type",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="type",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="email",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="email",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="address",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="address",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="zip_code",
                type_=AttributeType.INTEGER,
                array=False,
                optional=True,
                sortable=True,
                filter_name="zip_code",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="town",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="town",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="country",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="country",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="require_person_add_requests",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=True,
                filter_name="require_person_add_requests",
                filter_types=["eq"],
            ),
            Attribute(
                name="self_registration_url",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="self_registration_url",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="self_registration_require_adult_consent",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=True,
                filter_name="self_registration_require_adult_consent",
                filter_types=["eq"],
            ),
            Attribute(
                name="archived_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=True,
                sortable=True,
                filter_name="archived_at",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="created_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=True,
                filter_name="created_at",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="updated_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=True,
                filter_name="updated_at",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="deleted_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=True,
                sortable=True,
                filter_name="deleted_at",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="logo",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="privacy_policies",
                type_=AttributeType.STRING,
                array=True,
                optional=True,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="contact",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="creator",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="updater",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="deleter",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="parent",
                related_type_singular="group",
                related_type_plural="groups",
                to_many=False,
            ),
            RelationType(
                relation_name="layer_group",
                related_type_singular="group",
                related_type_plural="groups",
                to_many=False,
            ),
            RelationType(
                relation_name="phone_numbers",
                related_type_singular="phone_number",
                related_type_plural="phone_numbers",
                to_many=True,
            ),
            RelationType(
                relation_name="social_accounts",
                related_type_singular="social_account",
                related_type_plural="social_accounts",
                to_many=True,
            ),
            RelationType(
                relation_name="additional_emails",
                related_type_singular="additional_email",
                related_type_plural="additional_emails",
                to_many=True,
            ),
            RelationType(
                relation_name="mailing_lists",
                related_type_singular="mailing_list",
                related_type_plural="mailing_lists",
                to_many=True,
            ),
        ],
        includeable=[
            "contact",
            "creator",
            "updater",
            "deleter",
            "parent",
            "layer_group",
            "phone_numbers",
            "social_accounts",
            "additional_emails",
            "mailing_lists",
        ],
    ),
    "invoice_item": Entity(
        url="",
        name_singular="invoice_item",
        name_plural="invoice_items",
        string_representation_attributes=["invoice_id", "name"],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="invoice_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="description",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="unit_cost",
                type_=AttributeType.FLOAT,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="vat_rate",
                type_=AttributeType.FLOAT,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="cost",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="count",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="cost_center",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="account",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="invoice",
                related_type_singular="invoice",
                related_type_plural="invoices",
                to_many=False,
            )
        ],
        includeable=[],
    ),
    "invoice": Entity(
        url="/api/invoices",
        name_singular="invoice",
        name_plural="invoices",
        string_representation_attributes=["title"],
        allowed_operations=[
            Operation.GetList,
            Operation.GetSingle,
            Operation.CreateSingle,
        ],
        attributes=[
            Attribute(
                name="title",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="description",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="state",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="group_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name="group_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="recipient_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name="recipient_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="due_at",
                type_=AttributeType.DATE,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="issued_at",
                type_=AttributeType.DATE,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="recipient_email",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="payment_information",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="payment_purpose",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="hide_total",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="group",
                related_type_singular="group",
                related_type_plural="groups",
                to_many=False,
            ),
            RelationType(
                relation_name="recipient",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="invoice_items",
                related_type_singular="invoice_item",
                related_type_plural="invoice_items",
                to_many=True,
            ),
        ],
        includeable=["group", "recipient", "invoice_items"],
    ),
    "mailing_list": Entity(
        url="/api/mailing_lists",
        name_singular="mailing_list",
        name_plural="mailing_lists",
        string_representation_attributes=["name"],
        allowed_operations=[Operation.GetList, Operation.GetSingle],
        attributes=[
            Attribute(
                name="name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="name",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="group_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=True,
                filter_name="group_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="description",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="description",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="publisher",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="publisher",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="mail_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="mail_name",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="additional_sender",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="additional_sender",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="subscribable_for",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="subscribable_for",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="subscribable_mode",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="subscribable_mode",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="subscribers_may_post",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=True,
                filter_name="subscribers_may_post",
                filter_types=["eq"],
            ),
            Attribute(
                name="anyone_may_post",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=True,
                filter_name="anyone_may_post",
                filter_types=["eq"],
            ),
            Attribute(
                name="preferred_labels",
                type_=AttributeType.STRING,
                array=True,
                optional=False,
                sortable=True,
                filter_name="preferred_labels",
                filter_types=["eq"],
            ),
            Attribute(
                name="delivery_report",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=True,
                filter_name="delivery_report",
                filter_types=["eq"],
            ),
            Attribute(
                name="main_email",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="main_email",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="subscribable",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=True,
                filter_name="subscribable",
                filter_types=["eq"],
            ),
            Attribute(
                name="subscribers",
                type_=AttributeType.STRING,
                array=True,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="group",
                related_type_singular="group",
                related_type_plural="groups",
                to_many=False,
            )
        ],
        includeable=["group"],
    ),
    "person_name": Entity(
        url="",
        name_singular="person_name",
        name_plural="person_name",
        string_representation_attributes=["first_name", "last_name"],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="first_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="last_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[],
        includeable=[],
    ),
    "person": Entity(
        url="/api/people",
        name_singular="person",
        name_plural="people",
        string_representation_attributes=["first_name", "last_name"],
        allowed_operations=[
            Operation.GetList,
            Operation.GetSingle,
            Operation.CreateSingle,
        ],
        attributes=[
            Attribute(
                name="first_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="first_name",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="last_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="last_name",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="nickname",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="nickname",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="company_name",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="company_name",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="company",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=True,
                filter_name="company",
                filter_types=["eq"],
            ),
            Attribute(
                name="email",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="email",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="address",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="address",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="address_care_of",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="address_care_of",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="street",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="street",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="housenumber",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="housenumber",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="postbox",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="postbox",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="zip_code",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="zip_code",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="town",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="town",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="country",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="country",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="household_key",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="household_key",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="primary_group_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=True,
                filter_name="primary_group_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="gender",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="gender",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="birthday",
                type_=AttributeType.DATE,
                array=False,
                optional=False,
                sortable=True,
                filter_name="birthday",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="language",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="language",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="picture",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="picture",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="updated_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=True,
                filter_name="updated_at",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="additional_information",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="additional_information",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
        ],
        relations=[
            RelationType(
                relation_name="primary_group",
                related_type_singular="group",
                related_type_plural="groups",
                to_many=False,
            ),
            RelationType(
                relation_name="layer_group",
                related_type_singular="group",
                related_type_plural="groups",
                to_many=False,
            ),
            RelationType(
                relation_name="roles",
                related_type_singular="role",
                related_type_plural="roles",
                to_many=True,
            ),
            RelationType(
                relation_name="qualifications",
                related_type_singular="qualification",
                related_type_plural="qualifications",
                to_many=True,
            ),
            RelationType(
                relation_name="phone_numbers",
                related_type_singular="phone_number",
                related_type_plural="phone_numbers",
                to_many=True,
            ),
            RelationType(
                relation_name="social_accounts",
                related_type_singular="social_account",
                related_type_plural="social_accounts",
                to_many=True,
            ),
            RelationType(
                relation_name="additional_emails",
                related_type_singular="additional_email",
                related_type_plural="additional_emails",
                to_many=True,
            ),
            RelationType(
                relation_name="event_participations",
                related_type_singular="event_participation",
                related_type_plural="event_participations",
                to_many=True,
            ),
        ],
        includeable=[
            "primary_group",
            "layer_group",
            "roles",
            "qualifications",
            "phone_numbers",
            "social_accounts",
            "additional_emails",
            "event_participations",
        ],
    ),
    "phone_number": Entity(
        url="",
        name_singular="phone_number",
        name_plural="phone_numbers",
        string_representation_attributes=["contactable_id", "number"],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="public",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="contactable_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="contactable_type",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="number",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[],
        includeable=[],
    ),
    "qualification_kind": Entity(
        url="",
        name_singular="qualification_kind",
        name_plural="qualification_kinds",
        string_representation_attributes=[],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="description",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="validity",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="reactivateable",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="required_training_days",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[],
        includeable=[],
    ),
    "qualification": Entity(
        url="/api/qualifications",
        name_singular="qualification",
        name_plural="qualifications",
        string_representation_attributes=[],
        allowed_operations=[
            Operation.GetList,
            Operation.GetSingle,
            Operation.DeleteSingle,
        ],
        attributes=[
            Attribute(
                name="person_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name="person_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="qualification_kind_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name="qualification_kind_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="start_at",
                type_=AttributeType.DATE,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="finish_at",
                type_=AttributeType.DATE,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="qualified_at",
                type_=AttributeType.DATE,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="origin",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[
            RelationType(
                relation_name="person",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="qualification_kind",
                related_type_singular="qualification_kind",
                related_type_plural="qualification_kinds",
                to_many=False,
            ),
        ],
        includeable=["person", "qualification_kind"],
    ),
    "role": Entity(
        url="/api/roles",
        name_singular="role",
        name_plural="roles",
        string_representation_attributes=["person_id", "group_id", "type"],
        allowed_operations=[
            Operation.GetList,
            Operation.GetSingle,
            Operation.CreateSingle,
            Operation.DeleteSingle,
        ],
        attributes=[
            Attribute(
                name="created_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=True,
                filter_name="created_at",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="updated_at",
                type_=AttributeType.DATETIME,
                array=False,
                optional=False,
                sortable=True,
                filter_name="updated_at",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="start_on",
                type_=AttributeType.DATE,
                array=False,
                optional=True,
                sortable=True,
                filter_name="start_on",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="end_on",
                type_=AttributeType.DATE,
                array=False,
                optional=True,
                sortable=True,
                filter_name="end_on",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="name",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="person_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=True,
                filter_name="person_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="group_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=True,
                filter_name="group_id",
                filter_types=["eq", "not_eq", "gt", "gte", "lt", "lte"],
            ),
            Attribute(
                name="type",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=True,
                filter_name="type",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=True,
                sortable=True,
                filter_name="label",
                filter_types=[
                    "eq",
                    "not_eq",
                    "eql",
                    "not_eql",
                    "prefix",
                    "not_prefix",
                    "suffix",
                    "not_suffix",
                    "match",
                    "not_match",
                ],
            ),
        ],
        relations=[
            RelationType(
                relation_name="person",
                related_type_singular="person",
                related_type_plural="people",
                to_many=False,
            ),
            RelationType(
                relation_name="group",
                related_type_singular="group",
                related_type_plural="groups",
                to_many=False,
            ),
            RelationType(
                relation_name="layer_group",
                related_type_singular="group",
                related_type_plural="groups",
                to_many=False,
            ),
        ],
        includeable=["person", "group", "layer_group"],
    ),
    "self_registration": Entity(
        url="",
        name_singular="self_registration",
        name_plural="self_registrations",
        string_representation_attributes=["first_name", "last_name"],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="first_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="last_name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="nickname",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="email",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="adult_consent",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="privacy_policy_accepted",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[],
        includeable=[],
    ),
    "social_account": Entity(
        url="",
        name_singular="social_account",
        name_plural="social_accounts",
        string_representation_attributes=["contactable_id", "name"],
        allowed_operations=[],
        attributes=[
            Attribute(
                name="label",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="public",
                type_=AttributeType.BOOLEAN,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="contactable_id",
                type_=AttributeType.INTEGER,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="contactable_type",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
            Attribute(
                name="name",
                type_=AttributeType.STRING,
                array=False,
                optional=False,
                sortable=False,
                filter_name=None,
                filter_types=[],
            ),
        ],
        relations=[],
        includeable=[],
    ),
}

EntityName = typing.Literal[
    "additional_address",
    "additional_email",
    "course",
    "date",
    "event_guest",
    "event_kind_category",
    "event_kind",
    "event_participation",
    "event_role",
    "event",
    "group",
    "invoice_item",
    "invoice",
    "mailing_list",
    "person_name",
    "person",
    "phone_number",
    "qualification_kind",
    "qualification",
    "role",
    "self_registration",
    "social_account",
]

API_INFO = APIInfo(
    version="v1", auth_header="X-TOKEN", default_server_url="https://db.jubla.ch"
)
